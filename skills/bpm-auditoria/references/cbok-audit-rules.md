...
**Regra de Auditoria:** Qualquer representação do AS-IS que apresente notações ambíguas, baixa precisão ou ausência de uma convenção padronizada deve ser sinalizada como um achado na categoria `Notacao_Ambígua`.
...
```mermaid
graph TD
    N1["Nível 1: Corporativo (Perspectiva Executiva - Cadeia de Valor)"] --> N2["Nível 2: Negócio (Perspectiva de Processo - Ponta a Ponta)"]
    N2 --> N3["Nível 3: Fluxos de Trabalho (Perspectiva de Operações - Workflows)"]
    N3 --> N4["Nível 4: Passos das Tarefas (Perspectiva de Colaborador/Sistema - Instruções de Trabalho)"]
```
...
| `Desalinhamento_Hierarquico` | Mistura de níveis de processos (atividades micro em diagramas macro), falta de rastreabilidade entre estratégia e tarefas ou atribuição errada de responsabilidades. | Seções 4.6.4 / 4.7 |
...
O agente auditor de IA deve cumprir de forma intransigente as seguintes restrições em suas auditorias:
